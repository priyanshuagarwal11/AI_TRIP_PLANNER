...
import { ArrowRight, MessageSquare, Map, Bot, Edit3, Globe, Sparkles } from 'lucide-react';
import { TripFormWizard } from './TripFormWizard';

export const LandingPage = ({ onSubmit }: { onSubmit: (data: any) => void }) => {
  return (
    <div className="bg-[#0B1120] min-h-screen text-slate-300 font-sans overflow-hidden">
      
      {/* 0. WanderNav */}
      <nav className="absolute top-0 w-full z-50 border-b border-slate-800 bg-[#0B1120]/80 backdrop-blur-md">
        <div className="container mx-auto px-4 lg:px-8 py-5 flex items-center justify-between">
          <div className="text-2xl font-black font-serif text-yellow-500 hover:opacity-80 transition-opacity flex items-center gap-1 cursor-pointer tracking-tight">
            <span className="text-white">Wander</span>Mind
          </div>
          
          <div className="hidden lg:flex items-center gap-8 font-mono text-xs tracking-[0.15em] font-bold text-slate-400">
          ...
          <button 
            ...
            className="px-6 py-2.5 border border-yellow-500 text-yellow-500 font-mono text-xs font-bold tracking-widest hover:bg-yellow-500 hover:text-black transition-colors"
          >
            Start Planning →
          </button>
        </div>
      ...
      <section className="relative pt-40 pb-16 lg:pt-56 lg:pb-24 px-4 container mx-auto text-center flex flex-col items-center">
        ...
        <div className="border border-slate-700/50 bg-slate-800/30 backdrop-blur-md px-4 py-1.5 rounded-md mb-8 inline-flex items-center gap-2">
          <span className="w-2 h-2 rounded-full bg-blue-500"></span>
          <span className="text-xs font-mono tracking-widest text-slate-300 uppercase">AI-POWERED TRIP PLANNER</span>
        </div>

        <h1 className="text-6xl md:text-7xl lg:text-8xl font-bold text-white leading-[1.1] max-w-5xl font-serif">
        ...
        <p className="mt-8 text-lg md:text-xl text-slate-400 max-w-2xl text-center leading-relaxed">
      ...
      <section id="how-it-works" className="py-24 px-4 lg:px-12 xl:px-24 mx-auto max-w-[1400px]">
        ...
        <h2 className="text-4xl md:text-5xl font-serif text-white mb-20 tracking-tight">
          How <span className="italic text-yellow-500">WanderMind</span> works
        </h2>

        <div className="grid md:grid-cols-2 lg:grid-cols-4 border border-slate-800 rounded-sm overflow-hidden bg-slate-900/40">
          <div className="p-8 md:p-10 border-b lg:border-b-0 lg:border-r border-slate-800 hover:bg-slate-800/40 transition-colors">
            <div className="text-6xl font-black text-[#1e293b] mb-8 font-serif leading-none tracking-tight">01</div>
            <Map className="w-6 h-6 text-slate-300 mb-6" />
            <h3 className="text-xl font-serif text-white mb-4 font-bold tracking-wide">Tell us your vision</h3>
          ...
          <div className="p-8 md:p-10 border-b md:border-b-0 lg:border-r border-slate-800 hover:bg-slate-800/40 transition-colors">
            <div className="text-6xl font-black text-[#1e293b] mb-8 font-serif leading-none tracking-tight">02</div>
            <Bot className="w-6 h-6 text-slate-300 mb-6" />
            <h3 className="text-xl font-serif text-white mb-4 font-bold tracking-wide">AI builds your plan</h3>
            <p className="text-[#94a3b8] leading-relaxed text-[15px]">
              The model analyzes routes, hotels, restaurants, and attractions to build a perfectly timed itinerary.
            </p>
          </div>
          <div className="p-8 md:p-10 border-r border-slate-800 border-b lg:border-b-0 hover:bg-slate-800/40 transition-colors">
            <div className="text-6xl font-black text-[#1e293b] mb-8 font-serif leading-none tracking-tight">03</div>
            <Edit3 className="w-6 h-6 text-slate-300 mb-6" />
            <h3 className="text-xl font-serif text-white mb-4 font-bold tracking-wide">Refine & export</h3>
            <p className="text-[#94a3b8] leading-relaxed text-[15px]">
          ...
          <div className="p-8 md:p-10 hover:bg-slate-800/40 transition-colors">
            ...
            <Globe className="w-6 h-6 text-slate-300 mb-6" />
            <h3 className="text-xl font-serif text-white mb-4 font-bold tracking-wide">Travel smarter</h3>
            <p className="text-[#94a3b8] leading-relaxed text-[15px]">
      ...
      <section id="capabilities" className="py-24 px-4 lg:px-12 xl:px-24 mx-auto max-w-[1400px]">
        ...
        <div className="grid lg:grid-cols-2 gap-16">
          <div className="flex flex-col w-full">
            {[ 
              { num: '01', type: 'Smart Routing', title: 'Optimized day-by-day routes' },
              { num: '02', type: 'Budget AI', title: 'Real-time budget tracking' },
              { num: '03', type: 'Conversational UX', title: 'Natural language planning', active: true, desc: 'Say "Replace the museum with a cooking class" or "Add a beach day on Day 3" — WanderMind understands and updates instantly.' },
              { num: '04', type: 'Local Intelligence', title: 'Off-the-beaten-path gems' }
            ].map((cap, i) => (
              <div 
                key={i} 
                className={`py-8 px-6 lg:px-8 border-t border-slate-800 transition-all cursor-pointer ${cap.active ? 'border-l-[3px] border-l-yellow-500 bg-slate-900/30' : 'opacity-60 hover:opacity-100 hover:bg-slate-900/10 border-l-[3px] border-l-transparent'}`}
              >
                <div className="flex items-center gap-4 font-mono text-[11px] font-bold tracking-widest mb-4">
                  <span className={cap.active ? 'text-slate-400' : 'text-slate-500'}>{cap.num}</span>
                  <span className="text-slate-600">—</span>
                  <span className={cap.active ? 'text-slate-400' : 'text-slate-400'}>{cap.type}</span>
                </div>
                <h3 className="text-2xl font-serif text-white font-bold">{cap.title}</h3>
                {cap.active && cap.desc && (
                  <p className="mt-4 text-[#94a3b8] text-[15px] leading-relaxed max-w-md">{cap.desc}</p>
                )}
              </div>
            ))}
          ...
          <div className="bg-[#0f172a] border border-slate-800 rounded-sm relative flex flex-col items-center justify-center min-h-[450px] overflow-hidden lg:ml-8">
            ...
            <div className="relative z-10 flex flex-col items-center">
              <div className="w-16 h-16 bg-white rounded-t-2xl rounded-bl-2xl rounded-br-sm flex items-center justify-center shadow-[0_0_40px_rgba(255,255,255,0.1)] mb-8 animate-pulse">
              ...
              <div className="font-mono text-[#4b5563] text-xs font-bold tracking-[0.2em]">
                CHAT INTERFACE
              </div>
            </div>

            {/* Floating particles */}
            <div className="absolute top-1/4 left-[20%] w-1 h-1 bg-blue-500 rounded-full"></div>
            <div className="absolute bottom-[40%] right-[30%] w-1 h-1 bg-green-500 rounded-full"></div>
            <div className="absolute top-1/3 right-[25%] w-1 h-1 bg-yellow-500 rounded-full"></div>
            <div className="absolute bottom-[30%] left-[35%] w-1 h-1 bg-red-500 rounded-full"></div>
          </div>
        </div>
      </section>
      ...
      <section className="py-24 px-4 lg:px-12 xl:px-24 mx-auto max-w-[1400px]">
        ...
        <div className="rounded-sm border border-slate-800 bg-[#0B1120] overflow-hidden shadow-2xl relative">
          {/* Header OS style */}
          <div className="bg-[#0B1120] border-b border-slate-800 px-6 py-4 flex items-center gap-4">
            <div className="flex gap-2.5">
              <div className="w-3.5 h-3.5 rounded-full bg-[#ff5f56]"></div>
              <div className="w-3.5 h-3.5 rounded-full bg-[#ffbd2e]"></div>
              <div className="w-3.5 h-3.5 rounded-full bg-[#27c93f]"></div>
            </div>
            <div className="font-mono text-xs text-slate-500 flex-1 pl-4 tracking-wider">
          ...
          <div className="grid lg:grid-cols-2 divide-y lg:divide-y-0 lg:divide-x divide-slate-800">
            {/* Left Chat Frame */}
            <div className="p-8 lg:p-12 space-y-10 bg-[#0f172a]/50">
              
              <div className="flex flex-col items-end w-full space-y-2">
                <div className="bg-[#1e293b] p-6 rounded-sm text-[15px] text-slate-300 border border-slate-800/80 shadow-md max-w-[95%] leading-relaxed font-sans">
              ...
              <div className="flex flex-col items-start w-full space-y-2">
                <div className="bg-transparent border border-[#334155] p-6 rounded-sm max-w-[95%] relative">
                  ...
                  <p className="text-[15px] text-[#cbd5e1] leading-relaxed">
                    Perfect! Jaipur in 3 days on ₹15,000 — I've built a route that hits the grand forts at golden hour, skips tourist lunch traps, and ends each evening at a rooftop with a view. Here's your plan <ArrowRight className="inline w-4 h-4 text-slate-500 ml-1" />
                  </p>
                </div>
              </div>
            ...
            <div className="p-8 lg:p-12 space-y-8 bg-[#0B1120]">
              ...
              {[
                ...
                <div key={i} className="border border-[#1e293b] p-6 lg:p-8 hover:bg-[#0f172a] transition-colors rounded-sm">
                  <h4 className="font-mono text-xs tracking-[0.15em] font-bold text-yellow-500 mb-6 flex items-center gap-3">
                    {day.day} <span className="text-slate-600 font-sans font-light">—</span> {day.title}
                  </h4>
      ...
      <footer className="border-t border-slate-800/80 bg-[#0B1120] py-12 text-center text-slate-500 font-mono text-xs tracking-widest">
