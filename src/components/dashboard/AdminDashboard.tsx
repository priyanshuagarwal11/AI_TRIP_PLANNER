...
import { getAllUsers, getRecentActivities, getAdminStats, deleteUserData, updateUserRole, getAllGlobalTrips } from '../../lib/firestore';
...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ onBack, onLogout }) => {
  ...
  const [activeSection, setActiveSection] = useState<'overview' | 'users' | 'trips' | 'activity' | 'settings'>('overview');
  ...
  // Filter global trips
  const filteredTrips = globalTrips.filter(t => {
    const matchesSearch = (t.destination || '').toLowerCase().includes(tripSearchQuery.toLowerCase()) ||
                          (t.userName || '').toLowerCase().includes(tripSearchQuery.toLowerCase()) ||
                          (t.userEmail || '').toLowerCase().includes(tripSearchQuery.toLowerCase());
    const matchesDate = tripDateFilter ? formatTime(t.createdAt).includes(tripDateFilter) : true;
  ...
  useEffect(() => {
    ...
    const qUsers = query(collection(db, 'users'), orderBy('lastLogin', 'desc'), limit(100));
    const unsubUsers = onSnapshot(qUsers, (snap) => {
      setUsers(snap.docs.map(d => ({ id: d.id, ...d.data() })));
    });

    const qTrips = query(collection(db, 'trips'), orderBy('createdAt', 'desc'), limit(100));
    const unsubTrips = onSnapshot(qTrips, (snap) => {
      setGlobalTrips(snap.docs.map(d => ({ id: d.id, ...d.data() })));
    });

    const qActivities = query(collection(db, 'activities'), orderBy('timestamp', 'desc'), limit(50));
    const unsubActivities = onSnapshot(qActivities, (snap) => {
      setActivities(snap.docs.map(d => ({ id: d.id, ...d.data() })));
    });

    // Initial stats load (stats don't need real-time, but we can compute from local)
  ...
  return (
    <div className="flex min-h-screen bg-gray-50 dark:bg-slate-950">
                ...
                <div className="space-y-4 animate-in fade-in duration-300">
                  <div className="flex flex-col sm:flex-row gap-4 mb-4">
                    <div className="relative flex-1">
                      <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                      <input type="text" placeholder="Search by destination or user..." value={tripSearchQuery} onChange={e => setTripSearchQuery(e.target.value)} className="w-full bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl pl-10 pr-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500" />
                    </div>
                    <div className="relative w-full sm:w-48">
                      <input type="date" value={tripDateFilter} onChange={e => setTripDateFilter(e.target.value)} className="w-full bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500" />
                    </div>
                  </div>
                  <div className="card overflow-hidden">
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        <thead><tr className="border-b border-gray-100 dark:border-gray-800 text-left">
                          <th className="px-5 py-3 font-semibold text-gray-500 dark:text-gray-400">Destination</th>
                          <th className="px-5 py-3 font-semibold text-gray-500 dark:text-gray-400">User</th>
                          <th className="px-5 py-3 font-semibold text-gray-500 dark:text-gray-400">Details</th>
                          <th className="px-5 py-3 font-semibold text-gray-500 dark:text-gray-400">Created At</th>
                          <th className="px-5 py-3 font-semibold text-gray-500 dark:text-gray-400">Actions</th>
                        </tr></thead>
                        <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
                          {filteredTrips.map(t => (
                            <tr key={t.id} className="hover:bg-gray-50/50 dark:hover:bg-gray-800/50">
                              <td className="px-5 py-3">
                                <div className="font-semibold text-gray-900 dark:text-white flex items-center gap-2">
                                  <MapPin className="w-4 h-4 text-blue-500" /> {t.destination}
                                </div>
                              </td>
                              <td className="px-5 py-3">
                                <div className="text-gray-900 dark:text-white font-medium">{t.userName}</div>
                                <div className="text-xs text-gray-500">{t.userEmail}</div>
                              </td>
                              <td className="px-5 py-3 text-gray-500">
                                {t.days} Days • ₹{t.cost?.total?.toLocaleString()}
                              </td>
                              <td className="px-5 py-3 text-gray-500">{formatTime(t.createdAt)}</td>
                              <td className="px-5 py-3">
                                <button onClick={() => setSelectedTrip(t)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold text-blue-600 bg-blue-50 hover:bg-blue-100 dark:text-blue-400 dark:bg-blue-950/30 dark:hover:bg-blue-900/50 transition-colors">
                    ...
                    {filteredTrips.length === 0 && <div className="py-12 text-center text-sm text-gray-400">No trips found</div>}
                  </div>
                </div>
              )}
                  ...
                  <div className="card overflow-hidden">
                    <div className="px-5 py-4 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center">
                      <h3 className="font-bold text-gray-900 dark:text-white flex items-center gap-2">
                        Activity Log
                        <span className="flex items-center gap-1.5 text-xs font-semibold text-emerald-500 bg-emerald-50 dark:bg-emerald-900/30 px-2 py-1 rounded-full">
                        ...
                        <div key={a.id} className="px-5 py-3 flex items-center gap-4">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="text-xs text-gray-400 mt-0.5">
                              ...
                              {a.meta?.destination && ` • 📍 ${a.meta.destination}`}
                            </div>
                            {a.meta?.deviceInfo && (
                              <div className="text-[10px] text-gray-500 mt-1 flex gap-2">
                                <span>📱 {a.meta.deviceInfo.platform}</span>
                                <span>🌐 {a.meta.deviceInfo.language}</span>
                                <span>🖥 {a.meta.deviceInfo.screenResolution}</span>
                              </div>
                            )}
                          </div>
                          <span className="text-xs text-gray-400 shrink-0">{formatTime(a.timestamp)}</span>
      ...
      {/* ━━━ ITINERARY MODAL ━━━ */}
      {selectedTrip && (
        <div className="fixed inset-0 bg-black/60 z-[100] flex items-center justify-center p-4 backdrop-blur-sm" onClick={() => setSelectedTrip(null)}>
          <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="sticky top-0 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md px-6 py-4 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center z-10">
              <div>
                <h3 className="text-xl font-extrabold text-gray-900 dark:text-white flex items-center gap-2">
                  <MapPin className="w-5 h-5 text-blue-500" /> {selectedTrip.destination}
                </h3>
                <p className="text-sm text-gray-500 mt-1">Planned by {selectedTrip.userName} ({selectedTrip.userEmail})</p>
              </div>
              <button onClick={() => setSelectedTrip(null)} className="p-2 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-full transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="p-6 space-y-6">
              <div className="grid grid-cols-3 gap-4">
                <div className="bg-gray-50 dark:bg-gray-800/50 p-4 rounded-xl text-center">
                  <div className="text-2xl font-bold text-gray-900 dark:text-white">{selectedTrip.days}</div>
                  <div className="text-xs text-gray-500 uppercase tracking-wider font-semibold mt-1">Days</div>
                </div>
                <div className="bg-gray-50 dark:bg-gray-800/50 p-4 rounded-xl text-center">
                  <div className="text-2xl font-bold text-gray-900 dark:text-white">₹{selectedTrip.cost?.total?.toLocaleString()}</div>
                  <div className="text-xs text-gray-500 uppercase tracking-wider font-semibold mt-1">Budget</div>
                </div>
                <div className="bg-gray-50 dark:bg-gray-800/50 p-4 rounded-xl text-center">
                  <div className="text-2xl font-bold text-gray-900 dark:text-white">{selectedTrip.travelers || 2}</div>
                  <div className="text-xs text-gray-500 uppercase tracking-wider font-semibold mt-1">Travelers</div>
                </div>
              </div>
              <div>
                <h4 className="font-bold text-gray-900 dark:text-white mb-4">Itinerary</h4>
                <div className="space-y-4">
                  {selectedTrip.itinerary?.map((day: any, idx: number) => (
                    <div key={idx} className="border border-gray-100 dark:border-gray-800 rounded-xl p-4">
                      <h5 className="font-bold text-blue-600 dark:text-blue-400 mb-2">Day {day.day}: {day.title}</h5>
                      <p className="text-sm text-gray-600 dark:text-gray-400 mb-3">{day.subtitle}</p>
                      <div className="space-y-2">
                        {day.places?.map((place: any, pIdx: number) => (
                          <div key={pIdx} className="flex gap-3 bg-gray-50 dark:bg-gray-800/50 p-3 rounded-lg">
                            <div className="w-12 h-12 rounded-lg bg-gray-200 dark:bg-gray-700 overflow-hidden shrink-0">
                              {place.image && <img src={place.image} alt="" className="w-full h-full object-cover" />}
                            </div>
                            <div>
                              <div className="text-sm font-bold text-gray-900 dark:text-white">{place.name}</div>
                              <div className="text-xs text-gray-500 mt-1">{place.description}</div>
                              <div className="text-xs font-medium text-blue-500 mt-1">🕒 {place.time} ({place.period})</div>
                            </div>
                          </div>
                        ))}
                      </div>
